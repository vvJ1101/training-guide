...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch('/showroom/api/auth/login', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        window.location.href = '/showroom/internal/dashboard'
      } else {
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-neutral-200/50 border border-neutral-100 p-6 md:p-8">
          <form onSubmit={handleSubmit} noValidate>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 min-h-[44px] bg-neutral-900 text-white text-[0.82rem] font-medium rounded-lg hover:bg-neutral-800 active:bg-neutral-950 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
