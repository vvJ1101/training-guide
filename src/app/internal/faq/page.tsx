    ...
    <div className="p-6 md:p-8 lg:p-10 max-w-3xl">
        ...
        {Object.entries(typeLabels).map(([k, v]) => (
          <button
            ...
            className={`px-3 py-1.5 text-[0.72rem] rounded-md border transition-colors font-normal ${
              activeType === k ? 'bg-neutral-900 text-white border-neutral-900' : 'bg-white text-neutral-600 border-neutral-200 hover:border-neutral-400'
            }`}
          >
            {v}
          </button>
      ...
      <div className="flex flex-wrap gap-1.5 mb-6">
        <button
          ...
          className={`px-3 py-1.5 text-[0.72rem] rounded-md border transition-colors font-normal ${
            activeDept === 'all' ? 'bg-neutral-900 text-white border-neutral-900' : 'bg-white text-neutral-600 border-neutral-200 hover:border-neutral-400'
          }`}
        >
          全部
        </button>
        {depts.map(d => (
          <button
            ...
            className={`px-3 py-1.5 text-[0.72rem] rounded-md border transition-colors font-normal ${
              activeDept === d.slug ? 'bg-neutral-900 text-white border-neutral-900' : 'bg-white text-neutral-600 border-neutral-200 hover:border-neutral-400'
            }`}
          >
            {d.name}
          </button>
