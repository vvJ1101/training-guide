...
export default function SearchPage() {
  ...
  const doSearch = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/showroom/api/search?${params}`)
  ...
  return (
    <div className="p-8 md:p-10 max-w-4xl">
      ...
      <div className="flex gap-3 mb-6">
        ...
        <button
          onClick={doSearch}
          className="px-6 py-2.5 bg-neutral-900 text-white text-[0.85rem] font-medium rounded-lg hover:bg-neutral-800 transition-colors"
        >
          搜索
        </button>
      </div>
