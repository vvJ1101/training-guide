...
export default function LearningPathsPage() {
  ...
  useEffect(() => {
    fetch('/showroom/api/learning-paths')
      .then(r => r.json())
  ...
  const create = async () => {
    ...
    await fetch('/showroom/api/learning-paths', {
  ...
  const remove = async (id: string) => {
    ...
    await fetch(`/showroom/api/learning-paths?id=${id}`, { method: 'DELETE' })
  ...
  return (
    <div className="max-w-[800px] mx-auto px-4 md:px-8 py-8 md:py-12">
      <h1 className="text-[1.4rem] font-semibold tracking-[-0.02em] text-neutral-900 mb-6">🎓 学习路径管理</h1>
