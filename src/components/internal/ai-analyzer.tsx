...
export function AiAnalyzer({ documentId, documentTitle, documentCategory, originalContent, onClose, onApply }: Props) {
  ...
  async function analyze(fb?: string) {
    ...
    try {
      const res = await fetch(`/showroom/api/documents/${documentId}/analyze`, {
        method: 'POST',
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[4vh]">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-[95vw] max-w-[1400px] h-[92vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-200 shrink-0">
          <h2 className="text-[0.95rem] font-semibold text-neutral-900">
            ✨ AI 解析 · {documentTitle}
          </h2>
                ...
                {draft && (
                  <>
                    ...
                    <button onClick={regenerate} disabled={loading}
                      className="px-3 py-1 text-[0.7rem] font-medium bg-neutral-900 text-white rounded-md hover:bg-neutral-800 disabled:opacity-50 transition-colors">
                      {loading ? '生成中...' : '🔄 重新生成'}
                    </button>
                  </>
                )}
            ...
            {!draft && !loading && !error ? (
              <div className="flex-1 flex items-center justify-center">
                <button onClick={startAnalyze}
                  className="px-6 py-3 bg-neutral-900 text-white text-[0.85rem] font-medium rounded-lg hover:bg-neutral-800 transition-colors">
                  ✨ 开始 AI 解析
                </button>
              </div>
            ) : loading ? (
              <div className="flex-1 flex items-center justify-center">
                <div className="text-center">
                  <div className="animate-spin w-8 h-8 border-2 border-neutral-300 border-t-neutral-900 rounded-full mx-auto mb-3" />
              ...
              <div className="flex-1 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <button onClick={startAnalyze} className="px-4 py-2 text-[0.78rem] bg-neutral-900 text-white rounded-md hover:bg-neutral-800">重试</button>
                </div>
              </div>
            ) : (
              <>
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-2">
                        ...
                        <div>
                          <p className="text-[0.65rem] font-medium text-amber-700 mb-1">
                            ⚠️ 风险提示 ({analysisMeta.riskAlerts.length})
                          </p>
                        ...
                        <div>
                          {analysisMeta.strengths.length > 0 && (
                            <div className="mb-2">
                              <p className="text-[0.65rem] font-medium text-emerald-700 mb-1">✅ 优势</p>
                              <ul className="space-y-0.5">
                          ...
                          {analysisMeta.weaknesses.length > 0 && (
                            <div>
                              <p className="text-[0.65rem] font-medium text-neutral-600 mb-1">📋 待改进</p>
                              <ul className="space-y-0.5">
