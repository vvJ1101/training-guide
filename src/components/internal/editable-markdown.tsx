    ...
    <figure className="my-10 relative group" onMouseEnter={() => setShowMenu(true)} onMouseLeave={() => setShowMenu(false)}>
      ...
      {showMenu && (
        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => setEditing(!editing)} className="px-2 py-1 text-[0.65rem] bg-white border border-neutral-200 rounded shadow-sm hover:bg-neutral-50">✏️</button>
          <button onClick={onDelete} className="px-2 py-1 text-[0.65rem] bg-white border border-red-200 rounded shadow-sm text-red-500 hover:bg-red-50">🗑</button>
        </div>
      )}
      {editing ? (
