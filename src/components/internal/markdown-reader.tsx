...
export function MarkdownReader({ doc, canManage: canManageProp, hideHeader }: { doc: Doc; canManage?: boolean; hideHeader?: boolean }) {
  ...
  async function saveContent() {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/showroom/api/documents/${doc.id}`, {
        method: 'PUT',
  ...
  function openMeta() {
    ...
    Promise.all([
      ...
      fetch(`/showroom/api/documents/${doc.id}`).then(r => r.json()),
  ...
  async function handleMeta(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => { setShowMeta(false); window.location.reload() }, 600)
      } else {
  ...
  async function handleDelete() {
    ...
    if (res.ok) window.location.href = '/showroom/internal/documents'
    ...
    <div className="max-w-[960px] mx-auto py-8 md:py-12 px-4 md:px-8 lg:px-14 relative">
          ...
          <div className="flex items-center gap-1 mt-4">
            ...
            <button
              ...
              className={`px-3 py-1.5 text-[0.72rem] rounded-md border transition-colors font-normal ${
                viewMode === 'condensed' ? 'bg-neutral-900 text-white border-neutral-900' : 'bg-white text-neutral-600 border-neutral-200 hover:border-neutral-400'
              }`}
            >
              精简版
            </button>
            <button
              ...
              className={`px-3 py-1.5 text-[0.72rem] rounded-md border transition-colors font-normal ${
                viewMode === 'full' ? 'bg-neutral-900 text-white border-neutral-900' : 'bg-white text-neutral-600 border-neutral-200 hover:border-neutral-400'
              }`}
            >
              原文
            </button>
            ...
            {isEditing ? (
              <>
                ...
                <button onClick={saveContent} disabled={saving}
                  className="px-4 py-1.5 text-[0.72rem] text-white bg-neutral-900 rounded-md hover:bg-neutral-800 transition-colors disabled:opacity-50">
                  {saving ? '保存中...' : '保存内容'}
                </button>
              </>
            ) : (
      ...
      {isEditing ? (
        ...
        <div className="flex-1">
          <p className="text-[0.7rem] text-neutral-400 mb-4">💡 点击任意文字直接编辑，改完点「保存内容」</p>
          ...
          {newFormat ? (
            ...
            <article className="doc-content max-w-none">
              <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
                ...
                img: ({ src, alt }) => src ? (<figure className="my-10" style={{ contentVisibility: 'auto', containIntrinsicSize: 'auto 500px' }}><img src={src.startsWith('/') ? `/showroom${src}` : src} alt={alt || '截图'} className="w-full max-w-full h-auto rounded-lg border border-neutral-200 shadow-sm" loading="lazy" /><figcaption className="text-[0.78rem] text-neutral-400 text-center mt-3">{alt || '操作截图'}</figcaption></figure>) : null,
            ...
            <>
              ...
              {analysis.warnings.length > 0 && (
                <Section title="常见错误 & 注意事项">
                  <ul className="space-y-2">{analysis.warnings.map((w, i) => (<li key={i} className="flex items-start gap-2 text-[0.9rem] text-neutral-700 font-normal"><span className="text-amber-500 shrink-0 mt-0.5">⚠</span><span>{w}</span></li>))}</ul>
                </Section>
              )}
